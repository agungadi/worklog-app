  ...
  tablename.DataTable({
    ...
    ajax: {
      ...
      data: function data(_data) {
        _data.unit_divisi = $('#filterCountry').find('option:selected').val();
        console.log(_data.unit_divisi);
    },
    },
    columnDefs: [{
      'targets': [5],
    ...
    {
      data: 'master_divisi.nama_divisi',
      name: 'unit_divisi'
    },
    {
        data: 'username',
  ...
  function renderData(id) {
    $.ajax({
      ...
      success: function success(result) {
        ...
        $('#editdivisi').val(result.data[0].unit_divisi).trigger('change.select2');
