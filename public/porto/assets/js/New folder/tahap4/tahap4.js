...
$(document).ready(function () {
  $('#countryFieldID,#editCountryFieldID').select2({
    'width': '100%',
    'placeholder': 'Pilih SubKomponen 3'
  });
  ...
  tablename.DataTable({
    ...
    'order': [[0, 'asc']],
    ajax: {
      url: route('tahap4.index'),
      data: function data(_data) {
        _data.subkomponen_id = $('#filterCountry').find('option:selected').val();
        console.log(_data.subkomponen_id);
      }
    },
    columnDefs: [{
      'targets': [3],
    ...
    columns: [{
      ...
      data: 'subkomponen.name',
      name: 'subkomponen_id'
    }, {
      data: function data(row) {
        var url = route('tahap4.edit', row.id);
        var data = [{
          'id': row.id,
          'url': url
        }];
        ...
        api
            ...
            .each(function (group, i) {
                if (last !== group) {
                    $(rows)
                        .eq(i)
                        .before('<tr class="group" ><td colspan="4">[Sub Komponen] ' + group + '</td></tr>');
  ...
  $(document).on('click', '.delete-btn', function (event) {
    ...
    var deleteStateUrl = route('tahap4.destroy', stateId);
    deleteItem(deleteStateUrl, '#tahap3Table', 'Indikator');
